						...
						<div className="flex flex-wrap items-start -mx-1.5 mt-2">
							{state.mostRatedGetaways.length > 0
								? state.mostRatedGetaways.map((el, idx) => {
										while (
											state.mostRatedGetaways.indexOf(
												el
											) < 4
										) {
											let location;
						...
						<div className="flex flex-wrap items-start -mx-1.5 mt-2">
							{state.featuredRomanticGetaways.length > 0
								? state.featuredRomanticGetaways.map((el) => {
										while (
											state.featuredRomanticGetaways.indexOf(
												el
											) < 4
										) {
											let location;
							...
							<div className="flex flex-wrap items-center justify-end overflow-hidden relative rounded-md lg:px-10 lg:py-12 lg:pr-20 mt-4 shadow-md lg:shadow-none">
								...
								<div className="w-full lg:w-9/12 rounded-md bg-white relative z-10 shadow-md overflow-hidden">
									<RegularListBox
										...
										owner={
											state.featuredList.owner.fullName
										}
						...
						<div className="flex flex-wrap items-start -mx-1.5 mt-2">
							{state.featuredAdventureGetaways.length > 0
								? state.featuredAdventureGetaways.map((el) => {
										while (
											state.featuredAdventureGetaways.indexOf(
												el
											) < 4
										) {
											let location;
						...
						<div className="flex flex-wrap items-start -mx-1.5 mt-2">
							{state.featuredGastronomicGetaways.length > 0
								? state.featuredGastronomicGetaways.map(
										(el) => {
											while (
												state.featuredGastronomicGetaways.indexOf(
													el
												) < 4
											) {
												let location;
				...
				<div className="container">
					<div className="flex flex-wrap items-center justify-start overflow-hidden relative rounded-md lg:py-12 lg:pr-20">
						...
						<div className="w-full lg:w-6/12 rounded-md bg-white relative z-10 lg:shadow-lg overflow-hidden pt-8 lg:px-8 lg:pb-8">
							<h2 className="mb-4">
								Escapades en parella, i molt més
							</h2>
							<p className="text-[16px]">
								Hola, som l'Andrea i en Juli, i et volem donar
								la benvinguda a Escapadesenparella.cat, el
								recomanador d'escapades en parella de referència
								a Catalunya. Busques{" "}
								<strong>escapades en parella</strong>? No
								sabeu&nbsp;
								<strong>què fer aquest cap de setmana</strong>?
								Cansats de fer sempre el mateix? A
								Escapadesenparella.cat tenim la sol·lució!
							</p>
							<p className="text-[16px]">
								Fa {foundationYears} anys vam començar a
								compartir les escapades en parella que fèiem
								arreu de Catalunya, amb l'objectiu de motivar a
								sortir a <strong>descobrir Catalunya</strong>, i
								donar a conèixer llocs únics per gaudir en
								parella, perquè crèiem, i seguim creient, que hi
								ha vida més enllà d'anar al cinema o veure
								Netflix al sofà.
							</p>
							<p className="text-[16px] mb-0">
								A dia d'avui estem encantats de poder seguir
								compartint amb tots vosaltres les{" "}
								<strong>
									millors escapades en parella a Catalunya
								</strong>
								, així que gràcies per ser aquí llegint aquesta
								nota. Perquè per nosaltres,
								Escapadesenparella.cat és molt més que escapades
								en parella; esperem transmetre't aquest
								sentiment!
							</p>
							<div className="mt-6">
								<a
