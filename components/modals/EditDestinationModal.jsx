...
const EditDestinationModal = ({
	...
	const handleFileUpload = async () => {
		...
		try {
			...
			let nextCloudCarouselUploaded = false;
			...
			if (destination.updatedCarouselImages) {
				...
				nextCloudCarouselUploaded = true;
			}
			...
			setDestination((prev) => ({
				...
				cloudCarouselImagesUploaded: nextCloudCarouselUploaded,
				isSubmitable: true,
			}));
		} catch (err) {
			console.error(err);
	...
	const submitDestination = async () => {
		...
		let destinationImage, destinationSponsorLogo, destinationCarouselImages;
		cloudImage !== ""
			? (destinationImage = cloudImage)
			: (destinationImage = image);
		cloudSponsorLogo !== ""
			? (destinationSponsorLogo = cloudSponsorLogo)
			: (destinationSponsorLogo = sponsorLogo);
		cloudCarouselImagesUploaded === true && cloudCarouselImages != null
			? (destinationCarouselImages = cloudCarouselImages)
			: (destinationCarouselImages = carouselImages);

		try {
			setSubmitError(null);
			await service.editDestination(
				id,
				slug,
				title,
				longTitle,
	...
	if (destination.blopSponsorLogo) {
		...
		sponsorLogoPreview = (
			<div className="m-2 relative w-48 h-auto overflow-hidden rounded-md border-8 border-white shadow">
				<img src={destination.sponsorLogo} />
			</div>
		);
	}
				...
				<div className="modal__body">
						...
						<div className="form__group">
							...
							>
								<input
									...
									id="isFeatured"
									className="mr-2"
									checked={Boolean(destination.isFeatured)}
					...
					<div className="form__group">
						<label htmlFor="textSeo" className="form__label">
							reviewText - Review de la destinació
						</label>
					...
					<div className="form__group">
						<label htmlFor="textSeo" className="form__label">
							mostLikedText - El que ens ha agradat més de la
							destinació
						</label>
					...
					<div className="form__group">
						<label htmlFor="textSeo" className="form__label">
							pointsOfInterestText - Punts d'interès de la
							destinació
						</label>
