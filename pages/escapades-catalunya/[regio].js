...
const RegioPage = ({
  ...
  if (
    (state.notFound &&
      !state.isFetching &&
      Object.keys(state.regioDetails).length === 0) ||
                ...
                <>
                  ...
                  <div className="border-t border-primary-100 pt-10 mt-10">
                    <div className="w-full md:w-8/12 xl:w-5/12 md:mx-auto">
                      {state.regioDetails.seoText}
                    </div>
                  </div>
                </>
              ) : (
                <p className="text-center mx-auto text-lg">
                  No s'han trobat escapades per aquesta regio.
...
export async function getStaticPaths() {
  ...
  const regio = await service.getRegions();

  const paths = regio.map((regio) => ({
...
export async function getStaticProps({ params }) {
  ...
  let error404 = regioDetails == undefined ? true : false;

  if (regioDetails == undefined) {
    return {
      props: {
        error404,
      },
