...
const AdminPanel = () => {
	...
	useEffect(() => {
		const fetchData = async () => {
			...
			let activities = await service.activities();
			let places = await service.getAllPlaces();
	...
	return (
		<>
			...
			<main className="bg-primary-100 p-6">
				...
				<div className="w-full flex flex-wrap items-stretch mt-6 -mx-3">
					...
					<div className="w-10/12 px-3">
						<div className="bg-white rounded border border-primary-300 p-5">
							<h2 className="uppercase text-sm font-normal tracking-wider">
								Llista de resultats - {state.activeTab}
							</h2>
